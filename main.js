...
function showData() {
  ...
  for (let i = 0; i < arr.length; i++) {
    dataTask += `
            <div class="task row border border-5 border-light bg-primary mb-5">
                <h2 class="text-white  col-7 mx-5 fs-1 fw-bold py-4" id="title">${arr[i].containtTask}</h2>
                <div class="col-4 py-4">
                    <button onclick="editTask (${i})" class="btn btn-success me-5 fs-2 px-3" id="update">update</butto>
                    <button onclick="deleteTask (${i})" class="btn btn-danger fs-2 px-3" id="delete">delete</button>
                </div>
            </div>
        `;
  }
